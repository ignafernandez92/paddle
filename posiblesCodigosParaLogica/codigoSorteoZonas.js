...
function generarEnfrentamientosConCanchasYHorarios(zona, clubes, horas, duracionEnfrentamiento) {
    ...
    for (let i = 0; i < parejas.length; i++) {
        for (let j = i + 1; j < parejas.length; j++) {
            ...
            const clubHorario = asignarClubYHorario(enfrentamientos, clubes, horas, horarioBase, duracionEnfrentamiento);
...
function asignarClubYHorario(enfrentamientos, clubes, horas, horarioBase, duracionEnfrentamiento) {
    const disponibles = [];
    const clubIndex = Math.floor(Math.random() * clubes.length);

    for (const horario of horas) {
        let superposicion = false;

        // Verifica si hay superposiciones con enfrentamientos existentes
...
// DEFINITIVO CON 3 CLUBES POR AHORA
// Mejoro la legibilidad de la tabla, sacando datos innecesarios
//Orden cronologico para leer dentro de la zona, para facilitar lectura
//No solucione todavia la superposicion de hoarios